...
__global__ void T_ker(int *a, int *c, int width, int height) {
...
void matrix_T(const int* h_a, int* h_c, int width, int height) {
    // int width = 150;
    ...
    T_ker<<<blocksPerGrid, threadsPerBlock>>>(d_a, d_c, width, height);
